  ...
  return (
    <>
      {/* HOME */}
        <NavLink to={'/'} 
          ...
          ${isActive ? `isActiveStyle`: `` }`}>
            {({isActive})=> (
              <>
                HOME 
                <span className={isActive ? `text-[var(--color-accent)]` : ''}><DropDown /></span>
              </>
            )}
        ...
        {/* Blog */}
        <NavLink to={'/blog'} className={({isActive})=> `centering mr-2 ${isActive ? 'isActiveStyle' : ''}`}>
          BLOG
        </NavLink>

        {/* Contact */}
        <NavLink to={'/contact'} className={({isActive})=> `centering  mr-2 ${isActive ? 'isActiveStyle' : ''}`}>CONTACT</NavLink>
    </>
  )
}
